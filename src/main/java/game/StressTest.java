...
public class StressTest {
	...
	public static void main(String[] args) {
		...
		Random random = new Random(20220619);
		Scanner scan = new Scanner(System.in);
		int step = 0;
		while (step < 200) {
			step++;
			System.out.println("[" + java.time.LocalTime.now() + "] " + "Current step is " + step);
			if ((step == 5) || (step == 30) || (step == 40) || (step == 60)
					|| (step == 200)) {
				System.out.println("[" + java.time.LocalTime.now() + "] " + "######### Checkpoint at step " + step
						+ " #########");
...
//		}
	...
	private static void killPlayer(Vector<StressTestPlayer> l, Random r) {
		...
		synchronized(l) { 
			if (!shutdownInProgress) {
				System.out.println("[" + java.time.LocalTime.now() + "] " + "Killing player " + player.playerid);
				System.out.println("[" + java.time.LocalTime.now() + "] " + "Wait up to 5 seconds when killing a player.");
				player.extprocess.destroyForcibly();
				l.remove(player);
	...
	private static void doFastMoves(Vector<StressTestPlayer> l, Random r) {
		for (int i = 0; i < 5; i++) {
			...
			synchronized(l) { 
				if (!shutdownInProgress) {
					System.out.println("[" + java.time.LocalTime.now() + "] " + "Fast move " + player.playerid + " " + direction);
	...
	private static void doSlowMoves(Vector<StressTestPlayer> l, Random r,
		...
		for (int i = 0; i < 3; i++) {
			StressTestPlayer player = l.elementAt(r.nextInt(l.size()));
			int direction = r.nextInt(5);
			System.out.println("[" + java.time.LocalTime.now() + "] " + "Plan to move " + player.playerid + " " + direction);
			System.out.println("[" + java.time.LocalTime.now() + "] " + "TA should press return to initiate the move...");
			s.nextLine();
			...
			System.out.println("[" + java.time.LocalTime.now() + "] " + "TA should now verify the move is done correctly");
			System.out.println("[" + java.time.LocalTime.now() + "] " + "TA should press return after verification");
			s.nextLine();
			System.out.println("[" + java.time.LocalTime.now() + "] " + "Continuing the test...");
		}
	}
}
